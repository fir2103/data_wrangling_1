...
``` r
litters_df <- janitor::clean_names(litters_df) #cleans up variable names
```
...
``` r
lotr_words <- read_excel("data/LotR_Words.xlsx", range="B3:D6")
lotr_words
```
...
``` r
dont_do_this <- read.csv("data/FAS_litters.csv")
```
